...
const ACCENTS = ["#4f46e5", "#2563eb", "#059669", "#db2777", "#ea580c", "#0891b2", "#7c3aed", "#dc2626"];

type Tab = "build" | "design" | "preview";
...
export default function FormBuilder(props: FormBuilderProps) {
  ...
  const [theme, setTheme] = useState<FormTheme>(props.initialTheme ?? DEFAULT_THEME);
    ...
    <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <a href="/dashboard" className="rounded-lg px-2 py-1 text-sm text-slate-500 hover:bg-slate-100">←</a>
          <input
            value={title}
      ...
      {/* Mobile tabs */}
      <div className="flex gap-1 border-b border-slate-200 bg-white px-4 lg:hidden">
        {(["build", "design", "preview"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={[
              "px-3 py-2 text-sm font-semibold capitalize transition",
              tab === t ? "border-b-2 border-brand-600 text-brand-600" : "text-slate-500",
            ].join(" ")}
          >
            {t}
          </button>
        ...
        <aside className="space-y-4">
          <div className={tab === "design" ? "block" : "hidden lg:block"}>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Accent color</p>
              ...
              <p className="mb-2 mt-4 text-xs font-semibold uppercase tracking-wide text-slate-400">Direction</p>
                ...
                {(["rtl", "ltr"] as const).map((d) => (
                  <button
                    ...
                    className={[
                      ...
                      theme.dir === d ? "border-brand-400 bg-brand-50 text-brand-700" : "border-slate-200 text-slate-500 hover:bg-slate-50",
                    ].join(" ")}
                  >
                    {d}
                  </button>
            ...
            <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Field settings</p>
      ...
      {showPublish && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onClick={() => setShowPublish(false)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-green-100 text-2xl text-green-600">🎉</div>
            ...
            <div className="mt-4 flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2">
              <input readOnly value={publicUrl} className="min-w-0 flex-1 bg-transparent px-2 text-sm text-slate-700 outline-none" />
              <button onClick={copyUrl} className="rounded-lg bg-grad-brand px-3 py-1.5 text-sm font-semibold text-white">
                {copied ? "Copied!" : "Copy"}
              </button>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              <a href={publicUrl} target="_blank" rel="noreferrer" className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                Open form
              </a>
              <button onClick={() => setShowPublish(false)} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
                Done
              </button>
            </div>
          </div>
    ...
    >
      {fields.length === 0 ? (
        <div className="flex h-72 flex-col items-center justify-center text-center">
          ...
          <p className="mt-1 text-sm text-slate-400">or tap a field type on the left to add it</p>
        </div>
      ) : (
